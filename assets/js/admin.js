...
// Re-initialize handlers after page load for product edit pages
if (typeof pagenow !== 'undefined' && pagenow === 'product') {
    jQuery(window).on('load', function() {
        console.log('Re-initializing product page handlers');
        
        // Re-bind BOM handlers
        jQuery('#vwpm-add-bom-row').off('click').on('click', function(e) {
            e.preventDefault();
            console.log('Add BOM button clicked');
            var $rows = jQuery('#vwpm-bom-rows');
            var bomIndex = $rows.find('tr').length;
            var template = jQuery('#vwpm-bom-row-template').html();
            template = template.replace(/INDEX/g, bomIndex);
            $rows.append(template);
        ...
        // Re-bind Tool handlers
        jQuery('#vwpm-add-tool-row').off('click').on('click', function(e) {
            e.preventDefault();
            console.log('Add tool button clicked');
            var $rows = jQuery('#vwpm-tools-rows');
            var toolIndex = $rows.find('tr').length;
            var template = jQuery('#vwpm-tool-row-template').html();
            template = template.replace(/INDEX/g, toolIndex);
            $rows.append(template);
